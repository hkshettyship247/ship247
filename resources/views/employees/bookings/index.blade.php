...
<section class="shadow-box mt-8">
    <div class="dashboard-detail-box">
        ...
        <section class="search-result mt-8 mb-12">
            ...
            <form class="default-form" action="{{ route('employee.bookings.index') }}" method="GET">
                    ...
                    @if(isset($bookings) && count($bookings)> 0 && $cancelledBookingsCount > 0)
                        @foreach ($bookings as $booking)
                            @if($booking->status == config('constants.BOOKING_STATUS_CANCELLED') )
                                @include('employees.partials._booking-detail-box', ['booking' => $booking, 'tab' =>"cancelled-tab" ])
                            @endif
                        @endforeach
                    @else
                        <div class="p-4 rounded-lg bg-gray-50">
