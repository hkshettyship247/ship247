...
<section class="shadow-box mt-8">
    <div class="dashboard-detail-box">
        <header>
            ...
            {{-- <div class="w-6/12">
                <form class="dashboard-searchbar" action="{{ route('employee.user.index') }}" method="GET">
            ...
            <div class="w-6/12 md:justify-end flex">
                <a href="{{route('employee.user.create')}}" class="default-button-v2">
        ...
        <section class="search-result mt-8 mb-12">
            ...
            <form class="default-form" action="{{ route('employee.user.index') }}" method="GET">
                    ...
                    <div class="flex justify-end items-center h-full">
                        ...
                        <div id="dropdownUser{{$user->id}}" class="hidden z-10  bg-white divide-y divide-gray-100 rounded-lg shadow w-44 top-0 right-0">
                            <ul class="py-2 text-sm text-gray-700" aria-labelledby="dropdownUserButton{{$user->id}}">
                                <li>
                                    <a href="{{route('employee.user.edit', ['userID' => $user->id])}}" class="block px-4 py-2 hover:bg-red-600 hover:text-white">
                                        <span>Edit</span>
                                    </a>
                                </li>
                                <li>
                                    {{-- <a href="{{route('employee.user.delete', ['userID' => $user->id])}}" class="block px-4 py-2 hover:bg-red-600 hover:text-white">
                                        <span>Delete</span>
                                    </a> --}}
                                    <form action="{{route('employee.user.delete', ["userID" =>$user->id])}}" method="post">
                                        @csrf
